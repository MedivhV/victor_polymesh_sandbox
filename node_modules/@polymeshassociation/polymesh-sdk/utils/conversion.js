 ...
 * @hidden
 */
function identitiesSetToIdentities(identitySet, context) {
    return [...identitySet].map(rawId => new internal_1.Identity({ did: identityIdToString(rawId) }, context));
}
 ...
 * @hidden
 */
function toCustomClaimTypeWithIdentity(data) {
    return data.map(item => {
        var _a;
        return ({
            name: item.name,
            id: new bignumber_js_1.default(item.id),
            did: (_a = item.identity) === null || _a === void 0 ? void 0 : _a.did,
        });
    });
}
exports.toCustomClaimTypeWithIdentity = toCustomClaimTypeWithIdentity;
/**
 * @hidden
 */
function toHistoricalSettlements(settlementsResult, portfolioMovementsResult, filter, context) {
    const data = [];
    const getDirection = (fromId, toId) => {
        ...
        if (fromId === toId) {
            return typesV1_1.SettlementDirectionEnum.None;
        }
        if (filterPortfolioId && fromId === filter) {
            return typesV1_1.SettlementDirectionEnum.Outgoing;
        }
        if (filterPortfolioId && toId === filter) {
            return typesV1_1.SettlementDirectionEnum.Incoming;
        }
    ...
    settlementsResult.data.legs.nodes.forEach(({ settlement }) => {
        const { createdBlock, result: settlementResult, legs: { nodes: legs }, } = settlement;
        const { blockId, hash } = createdBlock;
        data.push({
            blockNumber: new bignumber_js_1.default(blockId),
            blockHash: hash,
            status: settlementResult,
            accounts: legs[0].addresses.map((accountAddress) => new internal_1.Account({ address: keyToAddress(accountAddress, context) }, context)),
    ...
    portfolioMovementsResult.data.portfolioMovements.nodes.forEach(({ createdBlock, from, to, fromId, toId, assetId, amount, address: accountAddress }) => {
        const { blockId, hash } = createdBlock;
        data.push({
            blockNumber: new bignumber_js_1.default(blockId),
            blockHash: hash,
            status: types_1.SettlementResultEnum.Executed,
            accounts: [new internal_1.Account({ address: keyToAddress(accountAddress, context) }, context)],
            legs: [
                {
                    asset: new internal_1.FungibleAsset({ ticker: assetId }, context),
                    amount: new bignumber_js_1.default(amount).shiftedBy(-6),
...
exports.toHistoricalSettlements = toHistoricalSettlements;
/**
 * @hidden
 */
function mediatorAffirmationStatusToStatus(rawStatus) {
    switch (rawStatus.type) {
        case 'Unknown':
            return { status: types_2.AffirmationStatus.Unknown };
        case 'Pending':
            return { status: types_2.AffirmationStatus.Pending };
        case 'Affirmed': {
            const rawExpiry = rawStatus.asAffirmed.expiry;
            const expiry = rawExpiry.isSome ? momentToDate(rawExpiry.unwrap()) : undefined;
            return { status: types_2.AffirmationStatus.Affirmed, expiry };
        }
        default:
            throw new utils_1.UnreachableCaseError(rawStatus.type);
    }
}
exports.mediatorAffirmationStatusToStatus = mediatorAffirmationStatusToStatus;
/**
 * @hidden
 */
function assetCountToRaw(counts, context) {
    return context.createType('PolymeshPrimitivesSettlementAssetCount', counts);
}
exports.assetCountToRaw = assetCountToRaw;
/**
 * @hidden
 */
function createRawExtrinsicStatus(status, value, context) {
    return context.createType('ExtrinsicStatus', { [status]: value });
}
exports.createRawExtrinsicStatus = createRawExtrinsicStatus;
