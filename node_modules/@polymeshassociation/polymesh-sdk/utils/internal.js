...
exports.compareTransferRestrictionToInput = exports.compareTransferRestrictionToStat = exports.compareStatsToInput = exports.assertTickerValid = exports.assertExpectedChainVersion = exports.extractProtocol = exports.assertExpectedSqVersion = exports.getExemptedIds = exports.defusePromise = exports.checkTxType = exports.getPortfolioIdsByName = exports.assembleBatchTransactions = exports.getCheckpointValue = exports.conditionsAreEqual = exports.hasSameElements = exports.toHumanReadable = exports.isModuleOrTagMatch = exports.optionize = exports.xor = exports.asFungibleAsset = exports.asAsset = exports.asBaseAsset = exports.asTicker = exports.assertAddressValid = exports.assertIsPositive = exports.assertIsInteger = exports.createProcedureMethod = exports.calculateNextKey = exports.requestAtBlock = exports.requestMulti = exports.getApiAtBlock = exports.requestPaginated = exports.isAllowedCharacters = exports.isPrintableAscii = exports.removePadding = exports.padString = exports.mergeReceipts = exports.sliceBatchReceipt = exports.segmentEventsByTransaction = exports.filterEventRecords = exports.createClaim = exports.getIdentity = exports.asDid = exports.asAccount = exports.asChildIdentity = exports.asIdentity = exports.getDid = exports.unserialize = exports.serialize = exports.delay = void 0;
exports.getAccount = exports.assertIdentityExists = exports.assertNoPendingAuthorizationExists = exports.areSameClaims = exports.asNftId = exports.assembleAssetQuery = exports.getIdentityFromKeyRecord = exports.getExemptedBtreeSet = exports.getSecondaryAccountPermissions = exports.assertStatIsSet = exports.neededStatTypeForRestrictionInput = exports.compareStatTypeToTransferRestrictionType = void 0;
 ...
 * @hidden
 */
function createClaim(claimType, jurisdiction, middlewareScope, cddId, customClaimTypeId) {
 ...
 */
function isAllowedCharacters(value) {
    return /^[0-9a-zA-Z_\-./]*$/.test(value);
}
exports.isAllowedCharacters = isAllowedCharacters;
/**
 * @hidden
 *
...
function asAsset(asset, context) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const fungible = new internal_1.FungibleAsset({ ticker: asset }, context);
        const collection = new internal_1.NftCollection({ ticker: asset }, context);
...
function conditionsAreEqual(a, b) {
    ...
    else if (aType === types_2.ConditionType.IsIdentity && bType === types_2.ConditionType.IsIdentity) {
        equalClaims = (0, conversion_1.signerToString)(a.identity) === (0, conversion_1.signerToString)(b.identity);
    }
    else if (aType === types_2.ConditionType.IsExternalAgent && bType === types_2.ConditionType.IsExternalAgent) {
        equalClaims = true;
    }
...
function getCheckpointValue(checkpoint, asset, context) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (type === types_2.CaCheckpointType.Existing) {
 ...
 * @hidden
 */
function assertExpectedNodeVersion(data, reject, isPrivateSupported) {
    const { result: version } = data;
    const neededMajor = isPrivateSupported
        ? (0, semver_1.major)(constants_1.PRIVATE_SUPPORTED_NODE_SEMVER).toString()
        : (0, semver_1.major)(constants_1.SUPPORTED_NODE_SEMVER).toString();
    const neededSemver = isPrivateSupported
        ? constants_1.PRIVATE_SUPPORTED_NODE_VERSION_RANGE
        : constants_1.SUPPORTED_NODE_VERSION_RANGE;
    if (!(0, semver_1.satisfies)(version, neededMajor)) {
    ...
    if (!(0, semver_1.satisfies)(version, neededSemver)) {
        console.warn(`This version of the SDK supports Polymesh RPC node version "${neededSemver}". The node is at version ${version}. Please upgrade the SDK`);
    }
}
/**
 * @hidden
 ...
 * @hidden
 */
function assertExpectedSpecVersion(data, reject, isPrivateSupported) {
    ...
    const specVersionAsSemver = addDotSeparator(specVersion)
        ...
        .join('.');
    const neededMajor = isPrivateSupported
        ? (0, semver_1.major)(constants_1.PRIVATE_SUPPORTED_SPEC_SEMVER).toString()
        : (0, semver_1.major)(constants_1.SUPPORTED_SPEC_SEMVER).toString();
    const neededSemver = isPrivateSupported
        ? constants_1.PRIVATE_SUPPORTED_SPEC_VERSION_RANGE
        : constants_1.SUPPORTED_SPEC_VERSION_RANGE;
    if (!(0, semver_1.satisfies)(specVersionAsSemver, neededMajor)) {
        const error = new internal_1.PolymeshError({
            code: types_2.ErrorCode.FatalError,
    ...
    if (!(0, semver_1.satisfies)(specVersionAsSemver, neededSemver)) {
        console.warn(`This version of the SDK supports Polymesh chain spec version "${neededSemver}". The chain spec is at version ${specVersionAsSemver}. Please upgrade the SDK`);
    }
}
/**
 * @hidden
...
function extractProtocol(url) {
    ...
    return (_a = matcher.exec(url)) === null || _a === void 0 ? void 0 : _a[1];
}
...
function assertExpectedChainVersion(nodeUrl) {
    return new Promise((resolve, reject) => {
        ...
        let confidentialAssetsSupported;
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        let nodeResponse;
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        let specResponse;
        const checkResponses = (cleanup) => {
        ...
        if (protocol.startsWith('ws')) {
            ...
            client.onopen = () => {
                client.send(JSON.stringify(constants_1.CONFIDENTIAL_ASSETS_SUPPORTED_CALL));
            ...
            client.onmessage = (msg) => {
                ...
                else if (id === constants_1.CONFIDENTIAL_ASSETS_SUPPORTED_CALL.id) {
            ...
            (0, cross_fetch_1.default)(nodeUrl, {
                method: 'POST',
                headers,
                body: JSON.stringify(constants_1.CONFIDENTIAL_ASSETS_SUPPORTED_CALL),
            })
            ...
            (0, cross_fetch_1.default)(nodeUrl, {
                method: 'POST',
                headers,
                body: JSON.stringify(constants_1.SYSTEM_VERSION_RPC_CALL),
            })
                .then(response => response.json())
                .then(data => {
                nodeResponse = data;
            ...
            (0, cross_fetch_1.default)(nodeUrl, {
                method: 'POST',
                headers,
                body: JSON.stringify(constants_1.STATE_RUNTIME_VERSION_CALL),
            })
                .then(response => response.json())
                .then(data => {
                specResponse = data;
...
function compareTransferRestrictionToStat(transferCondition, type, claimIssuer) {
    if ((type === types_2.StatType.Count && transferCondition.isMaxInvestorCount) ||
        (type === types_2.StatType.Balance && transferCondition.isMaxInvestorOwnership)) {
        return true;
    }
    if (!claimIssuer) {
        return false;
...
function compareTransferRestrictionToInput(rawRestriction, inputRestriction) {
    ...
    if (rawRestriction.isMaxInvestorCount && type === types_2.TransferRestrictionType.Count) {
        const currentCount = (0, conversion_1.u64ToBigNumber)(rawRestriction.asMaxInvestorCount);
        return currentCount.eq(value);
    }
    else if (rawRestriction.isMaxInvestorOwnership && type === types_2.TransferRestrictionType.Percentage) {
    ...
    else if (rawRestriction.isClaimCount && type === types_2.TransferRestrictionType.ClaimCount) {
    ...
    else if (rawRestriction.isClaimOwnership && type === types_2.TransferRestrictionType.ClaimPercentage) {
        const { min: valueMin, max: valueMax, claim: valueClaim, issuer: valueIssuer } = value;
...
function compareStatTypeToTransferRestrictionType(statType, transferRestrictionType) {
    ...
    if (opType === types_2.StatType.Count) {
        return transferRestrictionType === types_2.TransferRestrictionType.Count;
    }
    else if (opType === types_2.StatType.Balance) {
        return transferRestrictionType === types_2.TransferRestrictionType.Percentage;
    }
    else if (opType === types_2.StatType.ScopedCount) {
        return transferRestrictionType === types_2.TransferRestrictionType.ClaimCount;
    }
    else {
        return transferRestrictionType === types_2.TransferRestrictionType.ClaimPercentage;
    }
}
 ...
 * @hidden
 */
function areSameClaims(claim, { scope, type, customClaimTypeId }) {
    ...
    if ((0, typeguards_1.isScopedClaim)(claim) && scope && !(0, lodash_1.isEqual)((0, conversion_1.middlewareScopeToScope)(scope), claim.scope)) {
        return false;
    }
    if (type === types_1.ClaimTypeEnum.Custom &&
        claim.type === types_2.ClaimType.Custom &&
        customClaimTypeId &&
        !claim.customClaimTypeId.isEqualTo(customClaimTypeId)) {
        return false;
    }
    return types_2.ClaimType[type] === claim.type;
}
exports.areSameClaims = areSameClaims;
/**
 * @hidden
 */
function assertNoPendingAuthorizationExists(params) {
    const { authorizationRequests, message, authorization, target: targetToCheck, issuer: issuerToCheck, } = params;
    ...
    const pendingAuthorization = authorizationRequests.find(authorizationRequest => {
        ...
        if (targetToCheck && (0, conversion_1.signerToString)(target) !== (0, conversion_1.signerToString)(targetToCheck)) {
            return false;
        }
        if (issuerToCheck && (0, conversion_1.signerToString)(issuer) !== (0, conversion_1.signerToString)(issuerToCheck)) {
            return false;
        }
        if (authorization.type && data.type !== authorization.type) {
            return false;
        }
        if (authorization.type === types_2.AuthorizationType.PortfolioCustody && authorization.value) {
            const authorizationData = data;
            return authorizationData.value.isEqual(authorization.value);
        }
        if (authorization.type === types_2.AuthorizationType.AttestPrimaryKeyRotation && authorization.value) {
            const authorizationData = data;
            return authorizationData.value.isEqual(authorization.value);
        }
        // last checks for authorizations that have string values
        const { value } = authorization;
        const { value: authorizationValue } = data;
    ...
    if (pendingAuthorization) {
        ...
        throw new internal_1.PolymeshError({
            code: types_2.ErrorCode.NoDataChange,
            message,
            data: { target, issuer, authorizationType, authId },
        });
    }
}
exports.assertNoPendingAuthorizationExists = assertNoPendingAuthorizationExists;
/**
 * @hidden
 */
function assertIdentityExists(identity) {
    return __awaiter(this, void 0, void 0, function* () {
        const exists = yield identity.exists();
        if (!exists) {
            throw new internal_1.PolymeshError({
                code: types_2.ErrorCode.DataUnavailable,
...
exports.assertIdentityExists = assertIdentityExists;
/**
 * @hidden
 */
function getAccount(args, context) {
    return __awaiter(this, void 0, void 0, function* () {
        const { polymeshApi: { query: { multiSig }, }, } = context;
        const { address } = args;
        ...
        if (rawSigners.length > 0) {
            return new internal_1.MultiSig(args, context);
        }
