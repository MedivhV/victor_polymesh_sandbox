...
class Identity extends internal_1.Entity {
    ...
    hasValidCdd() {
        return __awaiter(this, void 0, void 0, function* () {
            const { context, did, context: { polymeshApi: { call }, }, } = this;
            const identityId = (0, conversion_1.stringToIdentityId)(did, context);
            const result = yield call.identityApi.isIdentityHasValidCdd(identityId, null);
    ...
    getPrimaryAccount(callback) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const assembleResult = (record) => __awaiter(this, void 0, void 0, function* () {
                const { primaryKey } = record.unwrap();
                const account = yield (0, internal_2.getAccount)({
                    address: (0, conversion_1.accountIdToString)(primaryKey.unwrap()),
                }, this.context);
                return {
                    account,
                    permissions: {
            ...
            if (callback) {
                context.assertSupportsSubscription();
                return identity.didRecords(rawDid, (records) => __awaiter(this, void 0, void 0, function* () {
                    const result = yield assembleResult(records);
    ...
    getHeldNfts(opts = {}) {
        return __awaiter(this, void 0, void 0, function* () {
            const { context, did } = this;
            const { size, start, order } = opts;
            const { data: { nftHolders: { nodes, totalCount }, }, } = yield context.queryMiddleware((0, queries_1.nftHoldersQuery)({
                identityId: did,
            }, size, start, order));
            const count = new bignumber_js_1.default(totalCount);
            const data = nodes.map(({ assetId: ticker, nftIds }) => {
                const collection = new internal_1.NftCollection({ ticker }, context);
                const nfts = nftIds.map((id) => new internal_1.Nft({ ticker, id: new bignumber_js_1.default(id) }, context));
    ...
    getSecondaryAccounts(args, callback) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const keyToAccount = (key) => __awaiter(this, void 0, void 0, function* () {
                const [, value] = key.args;
                const account = yield (0, internal_2.getAccount)({
                    address: (0, conversion_1.accountIdToString)(value),
                }, this.context);
                return account;
            });
            ...
            const accounts = yield Promise.all(keys.map(([key]) => keyToAccount(key)));
            if (cb) {
                context.assertSupportsSubscription();
    ...
    preApprovedAssets(paginationOpts) {
        return __awaiter(this, void 0, void 0, function* () {
            const { context, context: { polymeshApi: { query: { asset: { preApprovedTicker }, }, }, }, } = this;
            const rawDid = (0, conversion_1.stringToIdentityId)(this.did, context);
            const { entries, lastKey: next } = yield (0, internal_2.requestPaginated)(preApprovedTicker, {
                arg: rawDid,
                paginationOpts,
            });
            const data = yield Promise.all(entries.map(([storageKey]) => {
                const { args: [, rawTicker], } = storageKey;
                const ticker = (0, conversion_1.tickerToString)(rawTicker);
                return (0, internal_2.asAsset)(ticker, context);
            }));
            return { data, next };
        });
    }
    ...
    isAssetPreApproved(asset) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const rawTicker = (0, conversion_1.stringToTicker)(ticker, context);
            const rawDid = (0, conversion_1.stringToIdentityId)(this.did, context);
            const rawIsApproved = yield preApprovedTicker(rawDid, rawTicker);
            return (0, conversion_1.boolToBoolean)(rawIsApproved);
        });
    }
    /**
    ...
    getMultiSigSigners() {
        return __awaiter(this, void 0, void 0, function* () {
            const { context, context: { polymeshApi: { query: { multiSig: { multiSigToIdentity, multiSigSigners }, }, }, }, did, } = this;
            const entries = yield multiSigToIdentity.entries();
            const multiSigs = {};
            const rawSigners = yield Promise.all(entries
                .filter(([, rawIdentityId]) => {
                const identity = (0, conversion_1.identityIdToString)(rawIdentityId);
                return identity === did;
            })
                .map(([{ args: [rawMultiSigAccount], },]) => {
                const multiSigAccount = (0, conversion_1.accountIdToString)(rawMultiSigAccount);
                multiSigs[multiSigAccount] = [];
                return multiSigSigners.entries(rawMultiSigAccount);
            }));
            rawSigners.forEach(rawSigner => {
                rawSigner.forEach(([{ args: [rawMultiSigAccount, signatory], },]) => {
                    const multiSigAccount = (0, conversion_1.accountIdToString)(rawMultiSigAccount);
                    multiSigs[multiSigAccount].push((0, conversion_1.signerValueToSigner)((0, conversion_1.signatoryToSignerValue)(signatory), context));
                });
            });
            return Object.keys(multiSigs).map(multiSig => ({
                signerFor: new internal_1.MultiSig({ address: multiSig }, context),
