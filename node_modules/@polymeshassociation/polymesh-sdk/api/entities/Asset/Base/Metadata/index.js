...
class Metadata extends internal_1.Namespace {
    ...
    getDetails() {
        return __awaiter(this, void 0, void 0, function* () {
            const { context: { polymeshApi: { query: { asset: { assetMetadataValues, assetMetadataValueDetails, assetMetadataLocalKeyToName, assetMetadataGlobalKeyToName, assetMetadataGlobalSpecs, assetMetadataLocalSpecs, }, }, }, }, context, parent: { ticker }, } = this;
            const rawTicker = (0, conversion_1.stringToTicker)(ticker, context);
            const [rawValueEntries] = yield Promise.all([assetMetadataValues.entries(rawTicker)]);
            const namePromises = [];
            const specPromises = [];
            const valueDetailsPromises = [];
            rawValueEntries.forEach(([{ args: [, rawMetadataKey], },]) => {
            ...
            const nameValues = yield Promise.all(namePromises);
            const specValues = yield Promise.all(specPromises);
            const valueDetails = yield Promise.all(valueDetailsPromises);
            return rawValueEntries.map((rawValueEntry, index) => {
                const [{ args: [, rawMetadataKey], }, rawValue,] = rawValueEntry;
                return Object.assign({ metadataEntry: new internal_1.MetadataEntry(Object.assign(Object.assign({}, (0, conversion_1.meshMetadataKeyToMetadataKey)(rawMetadataKey, ticker)), { ticker }), context), name: (0, conversion_1.bytesToString)(nameValues[index].unwrap()), specs: (0, conversion_1.meshMetadataSpecToMetadataSpec)(specValues[index]) }, (0, conversion_1.meshMetadataValueToMetadataValue)(rawValue, valueDetails[index]));
            });
        });
    }
}
